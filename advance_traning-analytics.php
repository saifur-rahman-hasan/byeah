<?php include_once 'components/html_start.php'; ?>

		
<!-- Page Wrapper -->
<div id="wrapper">

	<!-- Navbar -->
	<?php include_once 'components/navbar.php'; ?>
	<!-- End Navbar -->

	<!-- Page Wrapper -->
	<div id="page-wrapper">
		<div class="container-fluid">
			
			<br>

			<div class="row">
				<div class="col-sm-12">
					
					<!-- Form - Mentoring club -->
					<form action="#" method="POST" autocomplete="off">
						<div class="panel panel-default">
							
							<div class="panel-heading">
								<h4>
									Analyics 
									<br>
									<small>
										Advanced Traning
									</small>
								</h4>
							</div>

							<div class="panel-body">
								
								<div class="row">
									<div class="col-sm-1 form-group">
										<label>Participants ID</label>
									</div>

									<div class="col-sm-2 form-group">
										<label>Participants Name</label>
									</div>
									<div class="col-sm-1 form-group">
										<label>Registration Batch No.</label>
									</div>
									<div class="col-sm-2 form-group">
										<label>Basic Traning Batch No.</label>
									</div>
									<div class="col-sm-2 form-group">
										<label>Pre-Traning Test Score</label>
									</div>
									<div class="col-sm-2 form-group">
										<label>Post-Traning Test Score</label>
									</div>
									<div class="col-sm-2 form-group">
										<label>Variannce</label>
									</div>
								</div>

								<div class="row">
									<div class="col-sm-1 form-group">
										<label>1</label>
									</div>

									<div class="col-sm-2 form-group">
										<label>ABC</label>
									</div>

									<div class="col-sm-1 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>
									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>
								</div>

								<div class="row">
									<div class="col-sm-1 form-group">
										<label>2</label>
									</div>

									<div class="col-sm-2 form-group">
										<label>ABC</label>
									</div>

									<div class="col-sm-1 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>
									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>
								</div>

								<div class="row">
									<div class="col-sm-1 form-group">
										<label>3</label>
									</div>

									<div class="col-sm-2 form-group">
										<label>ABC</label>
									</div>

									<div class="col-sm-1 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>
									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>
								</div>
								<div class="row">
									<div class="col-sm-1 form-group">
										<label>4</label>
									</div>

									<div class="col-sm-2 form-group">
										<label>ABC</label>
									</div>

									<div class="col-sm-1 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>
									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>
								</div>
								<div class="row">
									<div class="col-sm-1 form-group">
										<label>5</label>
									</div>

									<div class="col-sm-2 form-group">
										<label>ABC</label>
									</div>

									<div class="col-sm-1 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>

									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>
									<div class="col-sm-2 form-group">
										<input type="text" name="" class="form-control">
									</div>
								</div>

								
							</div>
							
							<div class="panel-footer text-center">
								<button type="submit" class="btn btn-sm btn-default">Share</button>
								<button type="submit" class="btn btn-sm btn-default">Print</button>
								<button type="submit" class="btn btn-sm btn-default">Download</button>
							</div>
						</div>
					</form>
					<!-- End Form - Mentoring club -->
				</div>		
			</div>

		</div>
	</div>
	<!-- End Page Wrapper -->

</div>
<!-- End Page Wrapper -->

<?php include_once 'components/html_stop.php'; ?>